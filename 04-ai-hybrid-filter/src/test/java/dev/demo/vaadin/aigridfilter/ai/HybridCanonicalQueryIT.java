...
class HybridCanonicalQueryIT {

    private static final Logger logger = LoggerFactory.getLogger(HybridCanonicalQueryIT.class);

    /**
     * What variant 04 can express — the whole point of this IT, and the only thing that differs from
     ...
     * <p>
     * Identical to 03's mapping on purpose: same filter type, different delivery. A divergence between
     * the two modules' results is therefore a statement about the mechanism, not about expressiveness.
     */
    private static Outcome outcomeOf(CanonicalQuery canonical) {
        return switch (canonical) {
            // 03's filter type, copied 1:1 — several values plus a negate flag per condition, and a
            // range as two sibling conditions on one field.
            case C1_SINGLE_VALUE, C2_MULTI_VALUE_OR, C3_NEGATION, C4_OPERATOR_PRECISION,
                 C5_COMBINED_AND, C6_REVENUE_RANGE, C7_RELATIVE_DATE, C8_DATE_RANGE -> Outcome.PASSES;
        };
    }
