 ...
 * <p>
 * A 1:1 copy of {@code 03-ai-structured-filter}'s test of the same (copied) translation logic — so if
 * the two modules ever diverge on a query, the cause can only be the delivery mechanism (tool call vs.
 * structured output), never the translation. Negation is split out into
 * {@link CustomerFilterSpecificationsExtraTest}, mirroring that module's split.
 * <p>
 * The multi-value (OR) and range cases below have no counterpart in {@code 02-ai-agent-filter}: neither
 * of its per-field criteria types can hold two values or two bounds for one field, which is exactly the
 * ceiling this module lifts while keeping tool calling as the delivery mechanism.
 */
@DataJpaTest
