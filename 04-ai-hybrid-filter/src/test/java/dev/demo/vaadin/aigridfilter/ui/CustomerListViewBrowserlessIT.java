 ...
 * the run fails rather than skipping, same as {@code CanonicalQueryIT}.
 * <p>
 * Uses the <em>same 7 queries</em>, method names, and source order as
 * {@code 03-ai-structured-filter}'s {@code CustomerListViewBrowserlessIT}, so the two modules'
 * {@code -Pit-local-ollama} runs are directly comparable: same filter type, same prompt rules, same
 * queries — the only difference is that the model calls a tool here instead of returning the filter as
 * structured output.
 * <p>
